...
const SIBLINGS = ['gym', 'daymark', 'fare', 'slate', 'research', 'degree'];
...
describe('shared Firestore rules parity check', () => {
  ...
  it('fails when only some sibling repositories are checked out', () => {
    ...
    expect(status).toBe(1);
    expect(stderr).toContain('only 2 of 6 sibling repositories are checked out');
    expect(stderr).toContain('fare, slate, research, degree');
    expect(stdout).toBe('');
  });
  ...
  it('reports a full comparison only when every sibling repository matched', () => {
    ...
    expect(status).toBe(0);
    expect(stdout).toContain('byte-identical copies in all 6 sibling repositories');
  });
